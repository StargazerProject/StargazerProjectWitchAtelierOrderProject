...
public class QueueUnitThread extends Thread {
	...
	@Override
	public void run() {
		while (PropertiesUtil.getValue("CenterControlMessageServiceMessageDisruptorStatus").equals("true")) {
			if (tasklist.size() == 0) {
				try {
				...
				logger.info("New Order Has ListingOut! Order Id is:" + order.OrderID().ID());
				producer.onData(order.OrderID().ID(),
						  order.OrderMessage().OrderMethod().Method().get(0).Parameter().get(0).Parameter(), "<p>PISUAL CELLS SYSTEM 指令ID: "
						+ order.OrderID().getID()
						+ "</p>下面将为您呈现执行本次指令的结果</p>"
						+ order.OrderMessage().OrderMethod().Method().get(0).Parameter().get(1).Parameter()+"</p> PISUAL CELLS SYSTEM 系统指令报告完毕</p>Noblesse Oblige 责任重大 愿您成为今天的救世主");
			}
		}
	}
}
