...
public class YandeCGMongoDBMethod implements YandeCGMongoDBMethodInterFace {
	...
	public static void main(String[] args) {
		...
		yandeCGMongoDBMethod.ensureIndexForTags();
		YandeCGGridFSMethod yandeCGGridFSMethod = new YandeCGGridFSMethod();
		List tags = new ArrayList();
		tags.add("eden");
		//tags.add("minori");
		List<YandeCG> result = yandeCGMongoDBMethod.querySpecifiedConditionsForTags(tags);
		int num = result.size();
		for(int i=0;i<num;i++)
		{
			YandeCGGridFS yandeCGGridFS = new YandeCGGridFS();
			yandeCGGridFS.setId(result.get(i).getId()+"");
			System.out.println(result.get(i).getTags());
			yandeCGGridFSMethod.getYandeCGFileFromMethod(yandeCGGridFS);
		}
}
}
